...
Add a composite action with a Bash helper owned by `cgraf78/actions` that expands the caller-owned
...
After the provider action lands, the reusable Rust release workflow calls it
at its immutable merge SHA instead of passing the whole glob to one
`gh release upload` invocation. The publish job remains
...
- Land the provider action before pinning it from the reusable workflow.
