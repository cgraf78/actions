...
RELEASE_STANDALONE_INSTALLER=true       # optional generated top-level install.sh
```
...
`RELEASE_STANDALONE_INSTALLER=true` opts into the standard self-contained
installer generated by [`release-installer/`](../release-installer/README.md).
It is deliberately not implied by `release.conf`: a repository with an existing
specialized installer retains ownership of that file. The normal
`consumer-ci/sync.sh` command generates and verifies the opted-in artifact from
the same Actions lock as these scripts.
...
2. Check out that commit and run `consumer-ci/sync.sh <consumer-repo>` for each
   consumer. Commit the lock, generated workflow refs, manifest, scripts, and
   opted-in top-level installer together.
